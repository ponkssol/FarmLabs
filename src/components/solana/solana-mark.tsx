type Props = {
  className?: string;
  /** Unique per instance to avoid duplicate SVG gradient ids. */
  gradientId: string;
};
...
/**
 * Resmi bentuk S / tiga bar (mark Solana yang dipakai di ekosistem) + gradien dekat palette brand.
 * Path dari referensi “simple-icons” / mark standar, diisi gradien karena rilis upstream hitam-putih.
 */
export function SolanaMark({ className, gradientId }: Props) {
  return (
    <svg
      className={className}
      viewBox="0 0 24 24"
    ...
    >
      <defs>
        <linearGradient id={gradientId} x1="0" y1="0" x2="24" y2="24" gradientUnits="userSpaceOnUse">
          ...
          <stop offset="0.45" stopColor="#00B7FF" />
          <stop offset="1" stopColor="#9C42FF" />
        </linearGradient>
      </defs>
      <path fill={`url(#${gradientId})`} d={SOLANA_MARK_PATH} />
    </svg>
  );
}
