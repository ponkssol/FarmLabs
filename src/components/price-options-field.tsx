...
const rowClass =
  "w-full rounded-md border border-white/10 bg-zinc-900 px-1.5 py-1 text-sm text-zinc-100 placeholder:text-zinc-600 focus:border-white/25 focus:outline-none sm:px-2 sm:py-1.5 sm:text-sm";
    ...
    <div className={className}>
      ...
      {showCurrency && (
        <div className="mt-2 max-w-xs">
          <span className="text-xs font-medium uppercase tracking-wide text-zinc-500">Currency (all tiers)</span>
          <select
            value={priceCurrency}
      ...
      <div className="mt-2 space-y-3">
        {value.length === 0 ? (
          <p className="text-xs text-zinc-600">No tiers yet — use the button below or fill the single price above.</p>
        ) : (
          value.map((row, i) => (
            <div
              key={row.id ?? `new-${i}`}
            ...
            >
              <div className="flex flex-col gap-1.5 sm:flex-row sm:items-end sm:gap-2">
                ...
                <div className="w-full sm:w-28">
                  <label className="text-xs font-medium uppercase text-zinc-500">Price</label>
                  <DecimalPriceInput
                    className={rowClass}
                ...
                <button
                  type="button"
                  onClick={() => removeRow(i)}
                  className="shrink-0 self-end rounded border border-white/10 px-2 py-1 text-xs text-zinc-500 hover:text-zinc-300"
                >
                  Remove
                </button>
              </div>
              <div className="grid gap-1.5 sm:grid-cols-2">
                <div>
                  <label className="text-xs font-medium uppercase text-zinc-500">Telegram (tier, optional)</label>
                  <input
                    className={rowClass}
                ...
                <div>
                  <label className="text-xs font-medium uppercase text-zinc-500">Discord invite (optional)</label>
                  <input
                    className={rowClass}
                ...
                <div>
                  <label className="text-xs font-medium uppercase text-zinc-500">Access days (optional)</label>
                  <input
                    type="number"
                    min={0}
                    max={3650}
                ...
                <div>
                  <label className="text-xs font-medium uppercase text-zinc-500">Discord role ID (optional)</label>
                  <input
                    className={rowClass}
        ...
        <button
          type="button"
          onClick={addRow}
          className="rounded border border-dashed border-white/15 px-2 py-1 text-xs text-zinc-400 hover:border-white/25 hover:text-zinc-200"
        >
          + Add access tier
        </button>
      </div>
